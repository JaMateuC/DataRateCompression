...
void quantitzationDifferential(float *, char *, float, float, int, char *);
...
void compressSignal(char *fileSig, char *filenameResult, char *fileNameDict)
{
    ...
    int dictSize, i = 0, leftintbin = 0;
   	...
   	float values[MAXDICT], real, imag, tempReal, tempImag, diffReal, diffImag;
    ...
    while(fgets(line, sizeof(line), file))
    {
        ...
        diffReal = real - tempReal;
        diffImag = imag - tempImag;
        printf("\n");
		printf("sample: %d-%d\n", i+1, i);
        ...
        quantitzationDifferential(&values[0], bitsCode, diffReal, diffImag, dictSize, samplestr);
...
void quantitzationDifferential(float *values, char *bitsCode, float sampleR, float sampleI, int dictSize, char *sampleStr)
{

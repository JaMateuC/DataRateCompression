...
void decompressSignal(char *, char *, char*, char *);
...
{
	char fileName[128],fileNameRes[128],fileNameDict[128], firstValue[128];
	
	if(argc == 5){
		
		strcpy(firstValue, argv[4]);
	    ...
	    decompressSignal(fileName,fileNameRes,fileNameDict, firstValue);
...
void decompressSignal(char *fileSig, char *filenameResult, char *fileNameDict, char *firstValue)
{
	int flagEnd = 0, ch, n = 0, idx = -1, dictSize, flagIQ = 0;
   	char bitsCode[MAXDICT][MAXBIT], newStr[MAXBIT], strrem[MAXBIT], sampleI[MAXBIT], sampleR[MAXBIT];
   	unsigned byte;
   	float values[MAXDICT], real, imag, tempImag, tempReal;
	...
	splitSample(firstValue, sampleI, sampleR);
	tempImag = strtof(sampleI,NULL);
    tempReal = strtof(sampleR,NULL);
    jointSample(fileResult, tempImag, tempReal);
